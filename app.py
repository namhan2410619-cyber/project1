...
if st.button("최적 경로 & 알람 계산"):
    ...
    if not start_loc or not end_loc:
        ...
        # 실제 ETA 반영
        if transport == "bus":
            bus_route_id = st.text_input("버스 노선 ID", "1000")
            station_id = st.text_input("정류장 ID", "12345")
            commute_time += get_bus_eta(bus_route_id, station_id)
        elif transport == "subway":
            line_no = st.text_input("지하철 호선", "1")
            commute_time += get_subway_eta(line_no)
